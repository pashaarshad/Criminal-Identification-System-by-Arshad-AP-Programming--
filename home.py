...
def showCriminalProfile(name):
    ...
    img = Image.fromarray(img)
    img = ImageTk.PhotoImage(img)
    img_label = tk.Label(scroll_frame, image=img, bg="#202d42")
    img_label.image = img
    img_label.grid(row=0, column=0, columnspan=2, pady=20)

    for i, item in enumerate(crim_data.items()):
        tk.Label(scroll_frame, text=item[0], pady=5, fg="yellow", font="Arial 12 bold", bg="#202d42").grid(row=i+1, column=0, sticky='w', padx=10)
        tk.Label(scroll_frame, text=":", fg="yellow", padx=10, font="Arial 12 bold", bg="#202d42").grid(row=i+1, column=1)
...
######################################## Home Page ####################################
# Create a canvas for the home page
home_canvas = tk.Canvas(pages[0], bg="#202d42", highlightthickness=0)
home_canvas.pack(side="left", fill="both", expand=True)

# Add a scrollbar to the canvas
home_scrollbar = tk.Scrollbar(pages[0], command=home_canvas.yview, width=20, troughcolor="#202d42", bd=0,
                              activebackground="#00bcd4", bg="#2196f3", relief="raised")
...
home_frame = tk.Frame(home_canvas, bg="#202d42")
home_canvas.create_window((0, 0), window=home_frame, anchor='nw')

# Update scrollregion when all widgets are in canvas
def on_home_configure(event):
    home_canvas.configure(scrollregion=home_canvas.bbox('all'))

home_canvas.bind('<Configure>', on_home_configure)

# Add widgets to the home_frame instead of pages[0]
tk.Label(home_frame, text="Criminal Identification System", fg="white", bg="#202d42",
         font="Arial 35 bold", pady=30).pack(anchor="center")

logo = tk.PhotoImage(file="logo.png")
tk.Label(home_frame, image=logo, bg="#202d42").pack(anchor="center")

btn_frame = tk.Frame(home_frame, bg="#202d42", pady=30)
btn_frame.pack(anchor="center")

tk.Button(btn_frame, text="Register Criminal", command=getPage1).pack(anchor="center")
tk.Button(btn_frame, text="Detect Criminal", command=getPage2).pack(anchor="center")
tk.Button(btn_frame, text="Video Surveillance", command=getPage3).pack(anchor="center")
...
for btn in btn_frame.winfo_children():
    ...
    btn.pack(pady=30, anchor="center")
